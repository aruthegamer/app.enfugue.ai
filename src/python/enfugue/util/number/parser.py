...
from enfugue.util.number.tokens_basic import Token, WordType
from enfugue.util.number.rules import CombinationRule, ConcatenationRule
from enfugue.util.number.text_processing_helpers import split_glues, find_similar_word
...
class NumberParser:
    ...
    >>> from enfugue.util.number import NumberParser
    >>> parser = NumberParser()
        ...
        for i, (word, glue) in enumerate(split_glues(text)):
            ...
            if self.similarity_threshold != 1:
                matched_num = find_similar_word(
                    word, Token.numwords.keys(), self.similarity_threshold
                )
