...
__all__ = ["merge_tokens", "merge_prompts"]
...
def merge_prompts(*prompts_no_weight: str, **prompt_weights: float) -> str:
    """
    Merges any number of prompts for a different library to parse later.
    """
    return str(TokenMerger(*list(prompts_no_weight)+list(prompt_weights.items())))
...
class TokenMerger:
    ...
    def clone(
        self,
        add_phrase: Optional[str] = None,
        add_weight: Union[int, float] = 1
    ) -> TokenMerger:
        """
