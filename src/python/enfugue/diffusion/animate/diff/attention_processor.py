...
class LoRAAttnProcessor(nn.Module):
    ...
    def __call__(
        ...
        key = attn.to_k(encoder_hidden_states) + lora_scale * self.to_k_lora(
            encoder_hidden_states
        )
        value = attn.to_v(encoder_hidden_states) + lora_scale * self.to_v_lora(
            encoder_hidden_states
        )
        ...
        # linear proj
        hidden_states = attn.to_out[0](hidden_states) + lora_scale * self.to_out_lora(
            hidden_states
        )
        # dropout
        hidden_states = attn.to_out[1](hidden_states)
...
class PoseAdapterAttnProcessor(nn.Module):
    def __init__(
        self,
    ...
    def forward(
        ...
        if encoder_hidden_states.ndim == 5:
            encoder_hidden_states = rearrange(
                encoder_hidden_states, "b c f h w -> (b f) (h w) c"
            )
        elif encoder_hidden_states.ndim == 4:
            encoder_hidden_states = rearrange(
                encoder_hidden_states, "b c h w -> b (h w) c"
            )
        else:
        ...
        attention_mask = attn.prepare_attention_mask(
            attention_mask, ehs_sequence_length, batch_size
        )
        ...
        if self.query_condition and self.key_value_condition:  # only self attention
            query_hidden_state = (
                self.qkv_merge(hidden_states + pose_feature) * pose_embedding_scale
                + hidden_states
            )
            key_value_hidden_state = query_hidden_state
        elif self.query_condition:
            query_hidden_state = (
                self.q_merge(hidden_states + pose_feature) * pose_embedding_scale
                + hidden_states
            )
            key_value_hidden_state = encoder_hidden_states
        else:
            key_value_hidden_state = (
                self.kv_merge(encoder_hidden_states + pose_feature)
                * pose_embedding_scale
                + encoder_hidden_states
            )
...
class LoRAPoseAdapterAttnProcessor(nn.Module):
    def __init__(
        self,
        hidden_size,  # dimension of hidden state
        pose_feature_dim=None,  # dimension of the pose feature
    ...
    def __call__(
        ...
        if self.query_condition and self.key_value_condition:  # only self attention
            query_hidden_state = (
                self.qkv_merge(hidden_states + pose_feature) * self.scale
                + hidden_states
            )
            key_value_hidden_state = query_hidden_state
        elif self.query_condition:
            query_hidden_state = (
                self.q_merge(hidden_states + pose_feature) * self.scale + hidden_states
            )
            key_value_hidden_state = encoder_hidden_states
        else:
            key_value_hidden_state = (
                self.kv_merge(encoder_hidden_states + pose_feature) * self.scale
                + encoder_hidden_states
            )
            query_hidden_state = hidden_states

        # original attention
        query = attn.to_q(query_hidden_state) + lora_scale * self.to_q_lora(
            query_hidden_state
        )
        key = attn.to_k(key_value_hidden_state) + lora_scale * self.to_k_lora(
            key_value_hidden_state
        )
        value = attn.to_v(key_value_hidden_state) + lora_scale * self.to_v_lora(
            key_value_hidden_state
        )
